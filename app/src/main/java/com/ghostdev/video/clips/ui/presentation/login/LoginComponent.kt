...
fun LoginComponent(
    ...
    navigateToHome: () -> Unit = {},
    viewmodel: LoginLogic = koinViewModel()
) {
    ...
    LaunchedEffect(key1 = state.value) {
        if (state.value.userLoggedIn || state.value.success) {
            navigateToHome()
        }
    }
...
fun LoginScreen(
    ...
    // Combine error from VM and local validation
        ...
        Column(
            ...
            Button(
                onClick = {
                    ...
                    when {
                        emailText.value.isEmpty() || passwordText.value.isEmpty() -> {
